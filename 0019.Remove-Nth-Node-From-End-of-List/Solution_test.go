...
func Test_removeNthFromEndRecursively(t *testing.T) {
	type args struct {
		head *ListNode
		n    int
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := removeNthFromEndRecursively(tt.args.head, tt.args.n); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("removeNthFromEndRecursively() = %v, want %v", got, tt.want)
			}
		})
	}
}