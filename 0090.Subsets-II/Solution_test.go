...
func Test_subsetsIteratively(t *testing.T) {
	type args struct {
		nums []int
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := subsetsIteratively(tt.args.nums); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("subsetsIteratively() = %v, want %v", got, tt.want)
			}
		})
	}
}
